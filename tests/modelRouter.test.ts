...
describe('selectModel', () => {
  it('starts on the strongest model when nothing has been used', () => {
    expect(selectModel({})).toEqual({ model: 'gemini-3.8-flash', remainingToday: 20 - HEADROOM - 1 });
  });

  it('steps down one model at a time as each budget fills', () => {
    // 19 used on a 20/day model = at the headroom line, so it moves on.
    expect(selectModel({ used: { 'gemini-3.8-flash': 19 } })?.model).toBe('gemini-3.7-flash');
    expect(
      selectModel({ used: { 'gemini-3.8-flash': 19, 'gemini-3.7-flash': 19, 'gemini-3.6-flash': 19 } })?.model,
    ).toBe('gemini-3.5-flash');
  });

  it('keeps one request of headroom per model', () => {
    // 18 of 20 still has room (that request leaves 19 used, the headroom).
    expect(selectModel({ used: { 'gemini-3.8-flash': 18 } })).toEqual({ model: 'gemini-3.8-flash', remainingToday: 0 });
    expect(selectModel({ used: { 'gemini-3.8-flash': 19 } })?.model).not.toBe('gemini-3.8-flash');
  });

  it('skips a model marked exhausted even when its counter looks fine', () => {
    const pick = selectModel({ used: {}, exhausted: { 'gemini-3.8-flash': true, 'gemini-3.7-flash': true } });
    expect(pick?.model).toBe('gemini-3.6-flash');
  });
  ...
  it('falls through to the lite models and then gives up', () => {
    ...
    delete allButLast['gemini-3.1-flash-lite'];
    expect(selectModel({ used: allButLast })).toEqual({ model: 'gemini-3.1-flash-lite', remainingToday: 500 - HEADROOM - 1 });
  });
